...
type msgWithResult struct {
	m      pb.Message // 消息输入
	result chan error // 处理结果
}

// 状态机的实现，对应一个节点
// node is the canonical implementation of the Node interface
type node struct {
	propc      chan msgWithResult // 向 raft StateMachine 递交一个 op propose
	recvc      chan pb.Message    // 向 raft StateMachine 递交 Peer 发送过来的一些 Message，例如一些 Response
